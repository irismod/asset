...
var (
	...
	maccPerms = map[string][]string{
		...
		token.ModuleName:                {auth.Minter, auth.Burner},
	}
...
func NewSimApp(
	...
	app.TokenKeeper = token.NewKeeper(appCodec, keys[token.StoreKey], app.subspaces[token.ModuleName], app.AccountKeeper, app.BankKeeper, auth.FeeCollectorName)
	...
	app.mm = module.NewManager(
		...
		transferModule,
		token.NewAppModule(app.TokenKeeper, app.AccountKeeper, app.BankKeeper),
	)
	...
	app.mm.SetOrderBeginBlockers(
		...
		evidence.ModuleName, staking.ModuleName, ibc.ModuleName, token.ModuleName,
	)
